...
export const DEFAULT_FORMULA_SEEDS: DefaultFormulaSeed[] = [
  ...
  {
    key: 'riskGauge',
    expression: 'MIN(100, 100*(w1*(legalExposureRatio/legalExposureRatioCap) + w2*MAX(0,MIN(1,scrutiny/100)) + w3*MIN(1,absOutrage/100) + w4*ownershipRisk + w5*solvencyRisk))',
    description: 'Overall risk score (0-100) — a weighted blend of legal exposure, scrutiny, outrage, majority-ownership takeover risk, and legal-solvency risk, each normalized to a 0-1 scale before weighting. The first three terms (w1/w2/w3) were the gauge\'s original design; the 4th (w4*ownershipRisk) and 5th (w5*solvencyRisk) are deliberate later additions — majority-ownership takeover and going bankrupt from open lawsuits are both fully independent ways to lose the game the original 3-term gauge never reflected — see CLAUDE.md\'s "Risk Gauge takeover term" and "Risk Gauge solvency term" sections. absOutrage is |outrage| (already non-negative, so its term only needs an upper clamp); scrutiny itself has no floor and can legitimately go negative (no decision drives it back up past 0 the way outrage-reducing decisions can with outrage), so its term is clamped on BOTH ends (MAX(0,MIN(1,...))) — the missing lower clamp here let the whole gauge dip below its documented 0-100 range whenever scrutiny went negative, a real bug found via random-play simulation. ownershipRisk and solvencyRisk are already pre-clamped to [0,1] in code (calculateOwnershipRisk/calculateSolvencyRisk), so neither is re-clamped here.',
  },
];
